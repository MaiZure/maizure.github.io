/*  BLK to BMP file converter designed for use with Traffic Department 2192 .BLK files
...
int main(int argc, char* argv[])
{
	...
	uint16_t IMAGE_WIDTH; 			/* Pixel width of a single frame*/
	uint16_t IMAGE_HEIGHT; 			/* Pixel height of a single frame*/
	...
	/* Open BLK and PAL File and find file size*/
	fin = fopen(argv[1],"rb");
	...
	IMAGE_WIDTH = 32;  	
	IMAGE_HEIGHT = 32;  
	IMAGE_COUNT = FILE_SIZE / (IMAGE_WIDTH * IMAGE_HEIGHT);
	...
	/* Extract image palette - 768 bytes*/
	...
	{	
		/* Extract 1k of image data, match to palette, write to surface */
